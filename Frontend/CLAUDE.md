...
- **화면 변형 확정안**: [`docs/screen_variants.md`](docs/screen_variants.md) — 7화면 변형 + 06 사이드바 매핑 + REQ-002 분산 의도 (디자인 핸드오프 v3, 2026-04-29 확정)
...
> 변형(A/B/C/D/E) 확정 매핑은 [`docs/screen_variants.md`](docs/screen_variants.md) 참조. 본 표는 라우트·데이터 소스 요약.

| 화면 | 라우트 | 주 데이터 소스 | 비고 |
|---|---|---|---|
| 랜딩 / 가입 안내 | `/` (비로그인) | 정적 마케팅 콘텐츠 | 비로그인 진입점, 인증 없이 접근 |
| 로그인 / 회원가입 | `/auth/*` | `POST /auth/login` (JWT httpOnly 쿠키) | OAuth 2.0 / SSO (REQ-007) |
| 대시보드 | `/` (로그인) | 월간 사용량 / 캐시백 추정 / 알림 카운트 | 첫 진입, 요약 위주 |
| 사용량 분석 | `/usage` | `power_1hour` 가전별 분해 + 시간대 패턴 | Recharts 라인/스택 차트 |
| 캐시백 | `/cashback` | 직전 2개년 동월 평균 대비 절감 + 단가 구간 산출 | 매월 1일/5일 배치 결과 |
| 설정 / 계정 | `/settings/*` | 프로필·한전·알림·보안·이상탐지내역·이메일 | 사이드바 6 탭 (`account/notifications/security/anomaly-log/email`) |
| AI 진단 | `/insights` | `appliance_status_intervals` 실시간 + LLM 추천 | 진단 리포트 (REQ-002) |

**REQ-002 이상 탐지 의도적 분산**:
- `/insights` (07) = 실시간 인사이트·추천 (모델 신뢰도, 주간 추이, 절약 권고)
- `/settings/anomaly-log` (06-D) = 과거 로그·감사 (전체 이벤트 테이블, 필터, CSV/JSON/PDF 내보내기)
- `/settings/email` (06-E) = 06-D 알림 채널 설정 (SMTP/POP, 수신자, 템플릿)

**Phase 0 제외**: 관리자 콘솔(`/admin/*`) — 후속 페이즈 별도 검토.
...
> 이 단계가 완료되기 전까지 핵심 7화면(Phase 1~7) 진입 금지.
...
- `src/main.tsx`, `src/App.tsx` — Router + AuthGuard 골격 + 7개 페이지 placeholder (랜딩/auth/대시보드/usage/cashback/settings/insights)
...
│   ├── features/          ← 도메인별 묶음 (landing, auth, dashboard, usage, cashback, settings, insights)
│   │   └── <domain>/
...
- **현재 = 데스크탑 1440 우선** — 2026-05-18 공모전 출품 데드라인 압박으로 웹 선행. 모바일 반응형은 출품 후 별도 PLAN_M 에서 일괄 도입 (그때부터 `<sm:` 모바일 우선 → `md:` 이상 데스크탑 보강 패턴 전환)
- **터치 타겟 ≥ 44×44px** — 모바일 단계 도입 시 강제 (현재는 권장)
- **WCAG 2.1 AA 준수** — `aria-*`, 색 대비 4.5:1, 키보드 포커스 명시 (현재 단계도 적용)
...
```tsx
<Route path="/" element={<LandingPage />} />            {/* 비로그인 */}
<Route path="/auth/*" element={<AuthRoutes />} />
<Route element={<AuthGuard />}>
  <Route path="/home" element={<DashboardPage />} />     {/* 로그인 후 대시보드 */}
...
> `/` 라우트가 비로그인(랜딩) / 로그인(대시보드) 으로 분기됨 — `AuthGuard` 가 인증 시 `/home` 으로 redirect 하는 패턴. 최종 라우트 분기 방식은 PLAN_00 에서 확정.
...
| E2E | Playwright | 핵심 7개 화면 골든 패스 | `tests/e2e/` |
