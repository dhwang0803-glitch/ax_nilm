# Review Agent 지시사항 (kpx-integration-settlement 브랜치)
...
- [ ] mock 데이터(HH001~HH003) 범위 이외의 household_id 처리 확인
- **판정 기준**: 재현 가능한 버그 시나리오 → Critical, 이론적 가능성 → Major

### 2. Error handling (실패 경로)
- [ ] diff에서 새로 추가된 외부 호출(OpenAI API / TimescaleDB IAP 터널 / FastAPI 의존성) 목록 작성
- [ ] 각 호출마다 try/except 또는 mock 폴백 존재 여부 확인
- [ ] OpenAI 호출: try-except + 단일 에이전트 폴백 순서 확인
- [ ] DB 호출: try-except + HH001~HH003 mock 폴백 확인
- [ ] 예외가 삼켜지지 않는지(`except: pass`) 확인
- **판정 기준**: 실패 시 500 에러 노출 또는 폴백 없음 → Critical, 로그만 빠짐 → Minor

### 3. Test coverage (변경 vs 테스트)
- [ ] 변경된 public 함수명/클래스명을 `tests/`에서 grep
- [ ] 매칭되는 테스트가 새 분기를 실제로 커버하는지 확인 (단순 import만 있는 경우 미커버)
- [ ] mock 폴백 경로와 정상 경로가 각각 테스트되는지 확인
- **판정 기준**: 신규 분기에 대응 테스트 0건 → Critical, 부분 커버 → Major

### 4. Performance
- [ ] TimescaleDB 쿼리가 라우터 루프 안에 있는지 (N+1 패턴)
- [ ] LLM API 호출이 중복되는지 (멀티에이전트 노드 간 캐싱 필요 여부)
- [ ] `get_*` 도구 함수가 반복 호출되지 않는지 (nilm_monitor 내 5개 도구)
- **판정 기준**: 운영 부하에서 실측 가능한 저하 → Major, 미세 → Minor

### 5. API / 인터페이스 설계
- [ ] 도구 함수 시그니처가 `TOOL_SCHEMAS`와 일치하는지 확인
- [ ] FastAPI 라우터 응답 스키마 일관성 (None vs 빈 dict vs 예외)
- [ ] `run_multi_agent()` / `run_coach()` / `run_insights()` 반환 타입 일관성
- [ ] `DEFAULT_HH` 환경변수로 household_id를 결정하는 라우터가 모든 엔드포인트에서 통일되었는지
