...
아래는 방향 힌트다. 시간대 이동은 총 kWh 절감 없으므로 제외.
NILM은 전력 파형만 측정하며 기기 내부 설정값(온도·화력 단계 등)은 알 수 없다.
따라서 title은 관찰된 소비 패턴(daily_kwh·peak_hours·사용 시간)을 근거로 작성한다.
설정 조정 방향은 사용 시간 단축·피크 회피·대기전력 절감으로 구체화할 것.
예) 에어컨 "설정 조정" → "저녁 에어컨 2시간 단축 X원" (관찰된 사용 시간 기반)
예) 전기장판 "설정 조정" → "취침 전기장판 1시간 단축 X원" (관찰된 야간 사용 기반)
{guidance}"""
