...
ADR-003 에 따른 NILM 학습 데이터 cross-region 접근 구성. (ADR-002 의 configurable dual-region 은 GCS 미지원으로 기각 — `docs/context/decisions.md` 참조.)
...
> 파일명이 `_dualregion_` 인 이유는 원래 ADR-002 기준으로 만들어졌기 때문. 실제 구성은 two-single-region. 필요 시 향후 rename (ADR-003 Follow-up).
...
ETL 파이프라인에는 이 세 단계를 묶는 헬퍼 스크립트를 추가한다 — ADR-003 Follow-up (`Database/scripts/sync_buckets.sh` 가칭).
...
- [ ] 1~5단계 전부 OK
- [ ] 팀원 검증 100%
- [ ] ADR-003 Follow-up 의 sync helper 스크립트 main 반영
