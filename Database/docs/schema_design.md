...
| Cold | 1일    | 7일 이상 과거 | `power_1day` (continuous aggregate on `power_1min`) |

- 신규 데이터는 매주(예: 월요일) 1주치를 1분 해상도로 `power_1min` 에 적재.
- 7일이 지난 chunk 는 **retention 정책으로 자동 삭제**, 그 전에 연속집계가
  `power_1day` 로 하루 단위 요약 저장 → 장기 보관.
- 집계 규칙(1분 → 1일)은 30Hz → 1분 때와 동일: `avg/min/max` + 누적 `energy_wh`.

### 0.1 이 결정의 이유

| 항목 | 30Hz 원시 | 1분 집계 (hot) | 1일 집계 (cold) |
|------|-----------|----------------|-----------------|
| 1채널/일 행 수 | 2,592,000 | 1,440 | 1 |
| 1채널/일 용량 | ~200 MB | ~115 KB | ~80 B |
| 전체 데이터셋 (110가구·31일·23채널) 기준 | ~8 TB | ~5~6 GB (전구간 1분 보관 시) | **~60 MB** (전구간 1일 보관 시) |
| 실제 DB 동시 점유 (7일 hot + 24일 cold) | — | ~1.1 GB (7일분) | ~50 MB (24일분) |
| NILM 엔진 접근 경로 | 파일/스트림 직접 (DB 우회) | — | — |

- 30Hz 원시는 NILM 엔진 내부 연산 중간 산출물 성격.
- 최근 7일은 대시보드·이상탐지 사후 조회용으로 1분 해상도 유지.
- 7일 이상 과거는 리포트·월간 추세용이라 1일 해상도로 충분, 저장 공간 1,440× 추가 절감.
...
| 1일 다운샘플 집계 (cold) | **TimescaleDB** continuous aggregate | `power_1min` 에서 자동 리프레시, 장기 보관 |
...
power_1day                     — 1일 다운샘플 (continuous aggregate, cold 장기)
...
1분 해상도 데이터는 압축이 아니라 **7일 retention + 1일 다운샘플**로 관리.

```sql
-- 1) 1일 다운샘플 연속집계 자동 리프레시 (매일)
...
**순서 안전성**: cagg 리프레시는 하루에 한 번 돌아 7일 전 데이터를 이미 요약해 둔 상태에서 retention drop 이 수행됨. cagg 가 멈추면 retention 도 멈춰야 데이터 손실 없음 → 운영 모니터링 필수.
...
| `energy_wh` | `sum` — 하루 총 소비전력량 (Wh) |
| `voltage_avg`, `current_avg`, `frequency_avg` 등 | `avg` (sample_count 균일 가정 시 근사) |
| `sample_count` | `sum` — 하루 원시 샘플 총수 (정상 2,592,000) |
| `minute_bucket_count` | `count(*)` — 집계 품질 지표 (정상 1,440) |
...
FROM power_1min
UNION ALL
SELECT day_bucket AS ts, household_id, channel_num,
       active_power_avg, energy_wh, '1day' AS resolution
FROM power_1day
WHERE day_bucket < NOW() - INTERVAL '7 days';  -- 중복 방지
```
...
| 기능 | 예상 추가 위치 |
|------|---------------|
| 1시간 집계 중간 계층 (대시보드 가속) | `power_1hour` cagg — `power_1min` 에서 파생 |
