...
|------|-----|
| GCP 프로젝트 | `ax-nilm` |
| 버킷 | `ax-nilm-data-dhwang0803` |
| 리전 | `asia-northeast3` (Seoul) |
| 데이터 경로 | `gs://ax-nilm-data-dhwang0803/nilm/training_dev10/` |
...
| 작업 | Egress | 비용 |
|------|--------|------|
| 전체 10가구(60 GB) 로컬 다운 | 60 GB | **$7.20** |
| 1가구 1가전(≈0.3 GB) 한 번 다운 | 0.3 GB | **$0.04** |
| 방식 1로 매 에폭 스트리밍 × 30에폭 × 1 GB | 30 GB | **$3.60** |
| GCE VM(같은 리전)에서 읽기 | — | **$0** |

단가: **인터넷 egress $0.12/GB, 같은 리전 GCP 내부는 $0**.

- 장기/대량 실험 예정이면 **GCE VM** 사용 고려 (비용 + 속도 모두 유리)
- 로컬 머신에서만 한다면 **방식 2 + 필요한 가구만**이 실전 공식
...
## 7. 참고: Validation / 전체 Training 데이터

현재 10가구 **개발 킷**만 업로드됨. 향후 확장 시:
- `gs://ax-nilm-data-dhwang0803/nilm/validation/` (16가구, Validation set — 예정)
- `gs://ax-nilm-data-dhwang0803/nilm/training_full/` (79가구 전체 Training — 예정)
